...
def get_mfcc_feature(data):
    ...
    try:
        ...
        ft5_trunc = np.hstack((np.mean(ft5, axis=1),
                               np.std(ft5, axis=1),
                               skew(ft5, axis=1),
                               np.max(ft5, axis=1),
                               np.median(ft5, axis=1),
                               np.min(ft5, axis=1)))
        ft6_trunc = np.hstack((np.mean(ft6), np.std(ft6), skew(ft6), np.max(ft6), np.median(ft6), np.max(ft6)))
        ft7_trunc = np.hstack((np.mean(ft7, axis=1),
                               np.std(ft7, axis=1),
                               skew(ft7, axis=1),
                               np.max(ft7, axis=1),
                               np.median(ft7, axis=1),
                               np.min(ft7, axis=1)))
        ft8_trunc = np.hstack((np.mean(ft8), np.std(ft8), skew(ft8), np.max(ft8), np.median(ft8), np.min(ft8)))
